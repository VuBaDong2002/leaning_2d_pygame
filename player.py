...
class Player(pygame.sprite.Sprite):
    ...
    def move(self, dt):
        ...
        # vertical
        if not self.on_surface['floor'] and any((self.on_surface['left'], self.on_surface['right'])) and not self.timers['wall slide block'].activate():
            self.direction.y = 0
        ...
        if self.jump:
            ...
            elif any((self.on_surface['left'], self.on_surface['right'])) and not self.timers['wall slide block'].activate():
                self.timers['wall jump'].activate()
                self.direction.y = -self.jump_height
    ...
    def flicker(self):
        if self.timers['hit'].active and sin(pygame.time.get_ticks() * 100):
