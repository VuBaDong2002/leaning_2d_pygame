...
from setting import *
from sprites import Sprite, MovingSprite, AnimatedSprite, Spike, Items, ParticleEffectSprites
...
class Level:
    ...
    def setup(self, tmx_map, level_frames):
        ...
        for  obj in tmx_map.get_layer_by_name('Items'):
            Items(obj.name, (obj.x + TILE_SIZE / 2, obj.y + TILE_SIZE / 2), level_frames['items'][obj.name], (self.all_sprites, self.item_sprites), self.data)
        ...
        for obj in tmx_map.get_layer_by_name('Water'):
            ...
            for row in range(rows):
                for col in range(cols):
                    ...
                    if row == 0:
                        AnimatedSprite((x,y), level_frames['water_top'], self.all_sprites, Z_LAYERS['main'])
                    else:
                        Sprite((x,y), level_frames['water_body'], self.all_sprites, Z_LAYERS['main'])
    ...
    def pearl_collision(self):
        for sprite in self.collision_sprites:
            ...
            if sprite:
                ParticleEffectSprites((sprite[0].rect.center), self.particle_frames, self.all_sprites)
        ...
        for sprite in self.damage_sprites:
            if sprite.rect.colliderect(self.player.hitbox_rect):
                ...
                if hasattr(sprite, 'pearl'):
                    sprite.kill()
                    ParticleEffectSprites((sprite.rect.center), self.particle_frames, self.all_sprites)
    ...
    def item_collision(self):
        if self.item_sprites:
            ...
            if item_sprites:
                ...
                ParticleEffectSprites((item_sprites[0].rect.center), self.particle_frames, self.all_sprites)
